"""Build the flat HELM zip, extract it, and smoke-test the extracted flat layout end-to-end
(the real grader entry: import myalgorithm; algorithm(prob, T)) in a subprocess whose cwd is
the extract dir -- exercising the flat import chain (myalgorithm -> helm_engine -> flux_engine
-> solver, all siblings) and the routed portfolio spawn.

    python tools/_helm_zip_smoke.py [inst:T ...]   (default: T1:60 T13:180 T20:180 T38:300)
"""
...
# 1. build the zip
zip_out = os.path.join(ROOT, "myalgorithm0703-helm.zip")
subprocess.run([PY, os.path.join(ROOT, "tools", "_build_helm_zip.py"),
                os.path.basename(zip_out)], cwd=ROOT, check=True)

# 2. extract to a temp dir
tmp = tempfile.mkdtemp(prefix="helm_zip_smoke_")
with zipfile.ZipFile(zip_out) as z:
    z.extractall(tmp)
print("extracted:", sorted(os.listdir(tmp)), flush=True)

# 3. write a __main__-guarded runner INSIDE the extract dir (flat) and run it there
#    (the portfolio uses multiprocessing 'spawn' -- see portfolio-spawn-guard).
...
jobs = sys.argv[1:] or ["T1:60", "T13:180", "T20:180", "T38:300"]
ok = True
for job in jobs:
    inst, T = job.split(":")
