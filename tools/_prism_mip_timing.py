...
for thr in (1,4):
    md=gp.Model(env=P.K._grb_env()); md.Params.OutputFlag=0; md.Params.Threads=thr; md.Params.Seed=0
    ...
    md.setObjective(w["w2"]*Mv+w["w3"]*pref,GRB.MINIMIZE); md.Params.TimeLimit=30
    t0=time.time(); md.optimize(); dt=time.time()-t0
    print(f"{inst} m={m} thr={thr}: status={md.Status} obj={md.ObjVal:.0f} gap={md.MIPGap:.4f} time={dt:.2f}s")
