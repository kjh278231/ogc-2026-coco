...
- **mask precompute 가속 = `shapely.prepare` (`SOLVER_MASK_PREPARE`, 기본 off) — 속도 ✓ 확정, 채택은 #1 대기** — 프로파일: 마스크 빌드의 **~92%가 셀별 점-포함 검사**(buffer 0.7%, bitloop 6.8%). `shapely.prepare(buf)` 한 줄로 그 단계 **−78%(4.5x)**, 마스크 빌드 총량 prob_20 21.6s→6.1s(**−72%**), **mismatch=0 = bit-identical**(feasibility 완전 보존, eval-count obj 동일). **그러나 full-20 wall 게이트(prepare on, more search): net −4.8%지만 위험 회귀** prob_12 **+38.8%**(plain unified −24%승→뒤집힘)·prob_15 +35.6%·prob_18 +20%·prob_6 +15% = **탐색 비단조(proxy-drift)**, prepare 버그 아님(고정 eval서 동일). unified 채택기준(최악 +4.9%)에 탈락 → **env-gate off, #1 후 flip.** adaptive-reserve·guided에 이어 **세 번째로 #1이 막는 벽** 확인. 출력 `.claude/scratch/{mask_profile,prepgate}/`.
