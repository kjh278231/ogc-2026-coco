...
class MaskProxy:
    ...
    def __init__(self, R, ix0, iy0, width_bits, height_rows, rows):
        self.R = R
...
def _local_mask(bd, o, R):
    ...
    for r in range(height_rows):
        ...
        for k in _np.nonzero(inside[r])[0]:
            rb |= (1 << int(k))
        rows.append(rb)
    m = MaskProxy(R, ix0, iy0, width_bits, height_rows, tuple(rows))
    _LOCAL_MASK[key] = m
    return m
