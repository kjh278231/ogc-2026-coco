...
def solve_bay(prob, j, ids, step=2, tcap=200, poly=False, deadline=None, mask=False, mask_R=8,
    ...
    `order` (multi-order packing, see solve_bay_best): explicit block placement order.
    Default None = the validated EDD order (due_date, processing_time) -> bit-identical."""
...
# --------------------------------------------------------------------------- #
# Multi-order best-of packing (env SOLVER_MULTIORDER). solve_bay places blocks in a
# FIXED EDD order; since tardiness is pure contention (the temporal floor is 0 on every
# train instance -- no block is late at its release), the placement ORDER decides who
# gets the scarce early slots, and EDD is not always best. The alternatives below each
# win different bays (validated tools/_order_probe.py: bay-oracle -24% Z1 on the hard
# family T20/14/13/11/18/9/6, `release` alone -19%). pack_best escalates to them ONLY when
# the EDD pack is tardy (most bays pack Z1=0 -> no overhead), keeps the min-tardiness
# placement, and ALWAYS includes EDD -> Pareto-safe (the result is never worse than EDD,
# feasibility is preserved as each candidate is a complete valid pack).
# --------------------------------------------------------------------------- #
# Which orders solve_bay_best tries, in order, EDD always first (Pareto-safe + bit-identical
# for EDD-only callers via [0]). Default = all 4 (the validated bay-oracle set). Set
# SOLVER_MO_ORDERS to a cheaper subset (e.g. "edd,release") to cut the per-tardy-bay pack cost
# -- `release` alone captured -19% of the -24% oracle, so 2 orders keep most of the win at ~half
# the throughput cost (the throughput-vs-quality knob for the multi-order portfolio workers).
_MO_ORDERS = [s.strip() for s in os.environ.get(
    "SOLVER_MO_ORDERS", "edd,release,leastslack,areadesc").split(",") if s.strip()]


def _order_candidates(prob, ids):
    """The multi-order set, EDD first (so EDD-only callers stay bit-identical via [0]).
    Honors SOLVER_MO_ORDERS (subset of edd,release,leastslack,areadesc); EDD always present."""
    b = prob["blocks"]
    def _area(i):
        mnx, mny, mxx, mxy = orient_bbox(b[i], 0)
        return (mxx - mnx) * (mxy - mny)
    builders = {
        "edd": lambda: sorted(ids, key=lambda i: (b[i]["due_date"], b[i]["processing_time"])),
        "release": lambda: sorted(ids, key=lambda i: (b[i]["release_time"], b[i]["due_date"])),
        "leastslack": lambda: sorted(ids, key=lambda i: (b[i]["due_date"] - b[i]["processing_time"],
                                                         b[i]["due_date"])),
        "areadesc": lambda: sorted(ids, key=lambda i: (b[i]["due_date"], -_area(i))),
    }
    names = [n for n in _MO_ORDERS if n in builders]
    if "edd" not in names:
        names = ["edd"] + names           # EDD must lead -> Pareto-safe, EDD-only bit-identical
...
def solve_bay_best(prob, j, ids, step=2, tcap=200, poly=False, deadline=None,
    ...
    tardy (T>0), also packs the alternative orders (release / least-slack / EDD+area-desc)
    and keeps whichever placement extracts the lowest tardiness. EDD is always in the set,
